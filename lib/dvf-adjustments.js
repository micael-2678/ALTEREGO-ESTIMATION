...
export function calculateAdjustedPrice(basePricePerM2, surface, adjustmentResult, dvfStats) {
  ...
  if (dvfStats.count >= 12) confidence += 20;
  if (dvfStats.months <= 12) confidence += 10;
  if (dvfStats.radius <= 400) confidence += 10;
  if (dvfStats.stdDev && dvfStats.stdDev > dvfStats.medianPricePerM2 * 0.3) {
    confidence -= 10; // Dispersion forte
  }
  ...
  // Bande d'incertitude
  let uncertaintyBand;
  if (confidence > 70) uncertaintyBand = 0.07;
  else if (confidence >= 50) uncertaintyBand = 0.09;
  else uncertaintyBand = 0.12;
