...
export function calculateAdjustments(characteristics, dvfStats, weights = ADJUSTMENT_WEIGHTS) {
  const adjustments = [];
  let totalImpact = 0;
  
  const isDense = dvfStats.count >= DRIFT_LIMITS.dense_threshold;
  ...
  // Bornes anti-dérive ASYMÉTRIQUES
  const maxPositiveImpact = isDense ? DRIFT_LIMITS.dense_max_positive : DRIFT_LIMITS.sparse_max_positive;
  const maxNegativeImpact = isDense ? DRIFT_LIMITS.dense_max_negative : DRIFT_LIMITS.sparse_max_negative;
  
  // Appliquer bornes asymétriques
  if (totalImpact > 0) {
    totalImpact = Math.min(totalImpact, maxPositiveImpact);
  } else {
    totalImpact = Math.max(totalImpact, -maxNegativeImpact);
  }
...
export function calculateAdjustedPrice(basePricePerM2, surface, adjustmentResult, dvfStats) {
  // Prix/m² après ajustements
  let adjustedPricePerM2 = Math.round(basePricePerM2 * (1 + adjustmentResult.totalImpact));
  
  // Appliquer offset global de calibration (-5%)
  if (CALIBRATION.apply_offset) {
    adjustedPricePerM2 = Math.round(adjustedPricePerM2 * (1 + CALIBRATION.global_offset));
  }
  ...
  // CLAMPING si confiance faible ou peu de comparables
  const shouldClamp = confidence < CALIBRATION.clamp_confidence_threshold || 
                      dvfStats.count < CALIBRATION.clamp_comparables_threshold;
  
  let clampApplied = false;
  if (shouldClamp) {
    const clampLow = basePricePerM2 * (1 - CALIBRATION.clamp_range);
    const clampHigh = basePricePerM2 * (1 + CALIBRATION.clamp_range);
    
    if (adjustedPricePerM2 < clampLow || adjustedPricePerM2 > clampHigh) {
      adjustedPricePerM2 = Math.round(Math.max(clampLow, Math.min(clampHigh, adjustedPricePerM2)));
      clampApplied = true;
    }
  }
  ...
  // Calcul delta vs DVF (pour monitoring)
  const deltaVsDvfPercent = ((adjustedPricePerM2 - basePricePerM2) / basePricePerM2 * 100).toFixed(2);
  ...
  return {
    ...
    deltaVsDvfPercent: parseFloat(deltaVsDvfPercent),
    clampApplied,
    offsetApplied: CALIBRATION.apply_offset ? CALIBRATION.global_offset * 100 : 0
  };
}
