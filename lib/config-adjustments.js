...
export const ADJUSTMENT_WEIGHTS = {
  ...
  floor_elevator: 10,        // ±10% max (conservé)
  
  // Extérieur (balcon, terrasse, jardin) - RÉDUIT
  outside: 8,                // ±8% max (était 10%)
  
  // Vue depuis le bien - RÉDUIT
  view: 6,                   // ±6% max (était 8%)
  
  // Stationnement - RÉDUIT
  parking: 4,                // ±4% max (était 6%)
  
  // État intérieur / standing - CONSERVÉ/RENFORCÉ
  condition: 12,             // ±12% max
  
  // DPE (performance énergétique) - RENFORCÉ
  dpe: 12,                   // ±12% max (était 10%)
  
  // Effet de surface (très petit/très grand) - RÉDUIT
  surface_effect: 6,         // ±6% max (était 8%)
  
  // Parcelle (maison uniquement) - RÉDUIT
  plot: 8,                   // ±8% max (était 10%)
  
  // Équipements maison (piscine, dépendance) - RÉDUIT
  house_extras: 5            // ±5% max (était 6%)
};
...
export const DRIFT_LIMITS = {
  // Zone dense (≥12 comparables)
  dense_max_impact: 0.25,    // ±25% maximum par rapport à la base DVF
  
  // Zone peu dense (<12 comparables)
  sparse_max_impact: 0.30,   // ±30% maximum
